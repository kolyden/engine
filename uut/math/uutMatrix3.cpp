...
namespace uut
{
	...
	Matrix3f Matrix3f::createMirror(const Vector3f &planeNormal)
	{
		...
		return Matrix3f(1.0f - 2.0f * x * x, -2.0f * y * x, -2.0f * z * x,
			-2.0f * x * y, 1.0f - 2.0f * y * y, -2.0f * z * y,
			-2.0f * x * z, -2.0f * y * z, 1.0f - 2.0f * z * z);
	}

	void Matrix3f::fromHeadPitchRoll(float headDegrees, float pitchDegrees, float rollDegrees)
	{
	...
	void Matrix3f::orient(const Vector3f &from, const Vector3f &to)
	{
		...
		else if (Math::closeEnough(e, -1.0f))
		{
			...
			Vector3f side(0.0f, from.z, -from.y);

			if (Math::closeEnough(Vector3f::dot(side, side), 0.0f))
				side.set(-from.z, 0.0f, from.x);

			side.normalize();

			Vector3f up = Vector3f::cross(side, from);
			up.normalize();
		...
		else
		{
			...
			float h = (1.0f - e) / Vector3f::dot(v, v);
	...
	void Matrix3f::rotate(const Vector3f &axis, float degrees)
	{
	...
	void Matrix3f::scale(float sx, float sy, float sz)
	{
	...
	void Matrix3f::toHeadPitchRoll(float &headDegrees, float &pitchDegrees, float &rollDegrees) const
	{
