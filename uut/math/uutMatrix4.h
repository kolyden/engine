...
namespace uut
{
	...
	public:
		...
		static Matrix4f createFromAxes(const Vector3f &x, const Vector3f &y, const Vector3f &z);
		static Matrix4f createFromAxesTransposed(const Vector3f &x, const Vector3f &y, const Vector3f &z);
		static Matrix4f createFromHeadPitchRoll(float headDegrees, float pitchDegrees, float rollDegrees);
		static Matrix4f createMirror(const Vector3f &planeNormal, const Vector3f &pointOnPlane);
		static Matrix4f createOrient(const Vector3f &from, const Vector3f &to);
		static Matrix4f createRotate(const Vector3f &axis, float degrees);
		static Matrix4f createScale(float sx, float sy, float sz);
		static Matrix4f createTranslate(float tx, float ty, float tz);
		...
		const float* GetData() const { return &mtx[0][0]; }
...
// 	protected:
	...
	inline Vector4f operator*(const Vector4f &lhs, const Matrix4f &rhs)
	{
		return Vector4f(
	...
	inline Vector3f operator*(const Vector3f &lhs, const Matrix4f &rhs)
	{
		return Vector3f(
	...
	inline Matrix4f operator*(float scalar, const Matrix4f &rhs)
	{
		return rhs * scalar;
	}

	inline Matrix4f Matrix4f::createFromAxes(const Vector3f &x, const Vector3f &y, const Vector3f &z)
	{
		Matrix4f tmp;
		tmp.fromAxes(x, y, z);
		return tmp;
	...
	inline Matrix4f Matrix4f::createFromAxesTransposed(const Vector3f &x, const Vector3f &y, const Vector3f &z)
	{
		Matrix4f tmp;
	...
	inline Matrix4f Matrix4f::createFromHeadPitchRoll(float headDegrees, float pitchDegrees, float rollDegrees)
	{
		Matrix4f tmp;
	...
	inline Matrix4f Matrix4f::createOrient(const Vector3f &from, const Vector3f &to)
	{
		Matrix4f tmp;
		tmp.orient(from, to);
		return tmp;
	...
	inline Matrix4f Matrix4f::createRotate(const Vector3f &axis, float degrees)
	{
		Matrix4f tmp;
	...
	inline Matrix4f Matrix4f::createScale(float sx, float sy, float sz)
	{
		Matrix4f tmp;
		tmp.scale(sx, sy, sz);
		return tmp;
	...
	inline Matrix4f Matrix4f::createTranslate(float tx, float ty, float tz)
	{
		Matrix4f tmp;
	...
	inline Matrix4f::Matrix4f(float m11, float m12, float m13, float m14,
		float m21, float m22, float m23, float m24,
		float m31, float m32, float m33, float m34,
		float m41, float m42, float m43, float m44)
	{
	...
	inline float *Matrix4f::operator[](int row)
	{
		return mtx[row];
	}

	inline const float *Matrix4f::operator[](int row) const
	{
		return mtx[row];
	}

	inline bool Matrix4f::operator==(const Matrix4f &rhs) const
	{
	...
	inline Matrix4f &Matrix4f::operator*=(float scalar)
	{
	...
	inline Matrix4f Matrix4f::operator*(float scalar) const
	{
		Matrix4f tmp(*this);
		tmp *= scalar;
		return tmp;
	...
	inline float Matrix4f::determinant() const
	{
	...
	inline void Matrix4f::fromAxes(const Vector3f &x, const Vector3f &y, const Vector3f &z)
	{
	...
	inline void Matrix4f::fromAxesTransposed(const Vector3f &x, const Vector3f &y, const Vector3f &z)
	{
	...
	inline void Matrix4f::toAxes(Vector3f &x, Vector3f &y, Vector3f &z) const
	{
	...
	inline void Matrix4f::toAxesTransposed(Vector3f &x, Vector3f &y, Vector3f &z) const
	{
